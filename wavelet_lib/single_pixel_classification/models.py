...
class PixelWiseClassifier(nn.Module):
    """Modello di rete neurale per la classificazione pixel-wise con trasformata scattering."""
    
    def __init__(self, in_channels, hidden_dim=128, num_classes=4):
        """
        ...
        self.conv3 = nn.Conv2d(hidden_dim, num_classes, kernel_size=1)
    ...
    def forward(self, x):
        ...
        Args:
            x: Tensore di input con coefficienti scattering
...
def train_pixel_classifier(
    ...
    if model is None or scattering is None:
        if config is None:
            raise ValueError("È necessario fornire config se model o scattering non sono forniti")
